...
def apply_mask(rows: list[dict], columns: list[str]) -> list[dict]:
    ...
    for row in rows:
        masked.append(
            {
                k: mask_value(k, v)
                if (k.lower() in cols or "password" in k.lower())
                else v
...
async def build_schema_snapshot(
    db: AsyncSession,
    ...
    if _schema_cache["expires_at"] > now:
        return (
            ...
            list(_schema_cache["tables"]),
            dict(_schema_cache["table_columns"]),
        )
    ...
    async with _schema_cache_lock:
        ...
        schema = "; ".join(
            f"{table}({', '.join(table_columns[table])})" for table in available_tables
        )
...
async def execute_query_request(
    ...
    rows, row_count, elapsed_ms = await query_executor.execute(
        db, query_request.generated_sql
    )
